...
from UseDatabase import register, check_status, set_status, statuses, remove_from_db, check_nick, nicks, set_nick, check_preds, preds, add_pred, remove_pred, marry, divorce, check_marriages, check_marriage
...
def marry_command(message, to):
    ...
    def create_keyboard():
        ...
        keyboard.add_button("Да", color=vk_api.keyboard.VkKeyboardColor.POSITIVE, payload={'type': 'text', 'text': 'zaebal'})
...
def marriages_list():
    ...
    def get_marriages():
        ...
        for i in range(0, len(marriages)):
            ...
            marriages_list.append(f"{i+1}. {name_or_nick(husband)} и {name_or_nick(wife)} ({marriage_time} дн.)")
        marriages_str = '\n'.join(marriages_list)
